...
class PlanOption extends Component {
        ...
        {this.props.paymentOptions.map((item, i) => {
          return (
            ...
            <p key={i + type + price} className="radio">
              <input type="radio" name="plan-details" value={(i + 1)} onClick={this.handlePaymentOptionRadioSelect.bind(null, item)} checked={item.is_option_selected} />
  ...
  renderPlan = (planList) => {
    ...
    let listProducts = moreProductOptions.map((moreProduct, index) => {
      ...
      return (<div className="span3" key={"itmVl" + index}>
        <div className="r-panel1" key={"itmVl1" + index}>
          ...
          <div className="input-prepend input-append cus-input default-margin-tp-btm">
            <span className="add-on" id="sizing-addon2">$</span>
            <input type="text" className="form-control" value={moreProduct.price - moreProduct.value} />
          </div>
    ...
    return (
      <div>
        ...
        <div className="span12">
          ...
          <button {...buttonAttr} className="btn btn-primary pull-right p-btn" onClick={this.handlePresentationPackage}>Presentation</button>
          <button {...buttonAttr} className="btn btn-default pull-right p-btn" onClick={this.saveData.bind(this,false)}>{this.state.printMenu ? 'Opening PDF...' : 'Print Menu'}</button>
          <button {...buttonAttr} className="btn btn-default pull-left p-btn" onClick={this.handleDeclinePackage.bind(this,true)}>Decline Packages</button>
          <button {...buttonAttr} className="btn btn-default pull-left p-btn" onClick={() => { this.handlePrintFinalMenu(true) }} target="_blank">{this.state.printFinalMenu ? 'Opening PDF...' : 'Print Final Menu'}</button>
        </div>
      </div>
    )
  }
