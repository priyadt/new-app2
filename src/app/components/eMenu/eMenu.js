...
class eMenu extends Component {
        ...
        HttpHelper(`${config.emenuMobileGatewayAPI}/dealer-products/`, 'get')
            .then((data) => {
                ...
                if (data.Products.length <= 0){
                  self.setState({showTermRate: true});
                }
                self.setState({
    ...
    eMenuOnsave(termRate) {
        return new Promise((resolve, reject) => {
            ...
            if (isvalidData) {
                ...
                this.setState({ saveEMenu: false },()=>resolve());
            }
        });
    }
    ...
    unhideTermRate(event){
      if (this.reqFieldsValidator()){
        this.setState({showTermRate: true, originalLoad: false, isError: false});
      } else{
    ...
    renderUI() {
        ...
        // }
            ...
            let ui = <div>{this.state.reqFieldResponseUI && <RequireProvider header="eMenu" error={this.state.isError} unhideTermRate={this.unhideTermRate.bind(this)} showTermRate={this.state.showTermRate}
                ...
                <div style={this.state.showTermRate ? {} : {display: 'none'}}><TermRate events={this.events.eMenuOnsave} editTermRate={this.editTermRate.bind(this)}
                  hasChangedTermOptionsData={this.hasChangedTermOptionsData} hasRenderedPackagePmt={this.hasRenderedPackagePmt} /></div>
