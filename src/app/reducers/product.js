...
export default function product(state = initialState, action) {
  switch (action.type) {
    ...
    case UPDATE_PRODUCT_PACKAGE_PRICE:
      ...
      newPackagePriceInfo.priceUpdateError = validatePriceUpdate( price, productId, state.list, newPackagePriceInfo.cost);
...
function updatePackageInfo(stat, payload) {
  let state = cloneDeep(stat);
    ...
    const priceToUpdate = parseInt(payload.price) + (oldPrice ? oldPrice.price : 0);
    const costToUpdate = parseInt(payload.cost) + (oldPrice ? oldPrice.cost : 0);
    if (levelChangePriceUpdate) {
      ...
      packageInfo = {
        ...
        cost: costToUpdate,
        price: priceToUpdate,
        priceUpdateError: validatePriceUpdate(priceToUpdate, payload.productId, state.list, costToUpdate)
      };
    }
...
function validatePriceUpdate( price, productId, products, cost) {
  ...
  const min = parseInt(product.min_price);
  const max = parseInt(product.max_price);
  if (max <= min) {
    if(price < cost){
      return `Price should not be less than Cost: $${cost}`;
    }
    return null;
    ...
    return `Price should be in the range of ${min} and ${max}.`;
  }
